...
| Phase 6: staging e2e green as a whole run | ⛔ **Cannot close: a real product defect reddens the run (2026-08-16, D-356).** Superseding the reading below. `video-intervention.spec.ts` fails about **one run in two** and is a **true positive**: `POST /respond` returns 200 with a real video, then a deferred `study_step` narrative frame — which omits `intervention`/`assistance_question` and replaces the client's whole snapshot — erases the panel ~1.5s later. Video and solution are the exposed branches because both *close* the pause, so only `intervention` holds the panel up. Characterised, not fixed: every quick fix would show a student help for a question they already left. **The earlier reading of this row was stale and is retracted.** It prescribed fixing "the walk's study-loop exit condition"; that shipped as D-340 and the drift survived it (2 of 8 runs), and D-340 §2 had already retracted the mechanism — `serverPhase` never left `study`. D-355 has since removed the three ways the walk could invent a drift by itself (clicks counted as answers, refusals dropped, refusals forgiven by an allowance that was never path-scoped), so any future drift is evidence about the server |
