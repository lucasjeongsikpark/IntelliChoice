...
- **A repository that commits independently of the request needs its tests built differently, and
  three of the four rules were learned by breaking them** (D-421). `chat_turn_cancellations` and
  `chat_escalation_sends` both commit outside the request's transaction — deliberately, because a
  cancellation must be visible to another ECS task and a send-claim must be visible *at the moment of
  the send* rather than at the end of the request. That single property invalidates four ordinary test
  habits at once: **fixed literal ids** make a suite pass exactly once (the claim outlives
  `rollback_session`, so the second run's "duplicate" is the first run's email); **an engine per helper
  call** leaks a connection pool and surfaces as failures in a *different app*, a whole suite away from
  the change; **a module-level engine** fails with `attached to a different loop`, because each test
  drives the graph through its own `asyncio.run` and asyncpg binds connections to the loop that opened
  them; and call sites that must never reach the repository want a **stand-in that raises**, not a real
  one, so "this turn does not escalate" is asserted rather than assumed. The working shape is a per-test
  `@asynccontextmanager` disposing in a `finally`, uuid-suffixed keys, and `UnusedEscalationSends`.

- **A grounded answer costs four sequential model round trips, and the split is not where reasoning
  puts it** (D-423). `SCOPE_AND_INTENT` → `create_embedding` → `RERANK` → `RAG_ANSWER`, measured on
  staging at **2129 / 124 / ~3300 / 4124 ms** — so the embedding is **1.3%** of a ~10s turn and the
  classification is 22%. Two consequences worth carrying: an optimisation argued from "four calls, ten
  seconds, so ~2.5s each" was wrong by two orders of magnitude for the stage it targeted, and the real
...
flowchart TB
    ...
    subgraph PKGS["Workspace packages"]
        ...
        DB["packages/db (S3)<br/>29 SQLAlchemy models · repositories<br/>hybrid_search + RRF (S13)<br/>mcp_tool_calls (S14)<br/>youtube_videos + search_catalog (S15)<br/>question_validation_runs (S20)<br/>hint_events (S21)<br/>tutor_chat_messages (S24)<br/>learning_events, semantic_memory +<br/>superseded_by_id/contradicts_event_count<br/>(S25)<br/>youtube_videos: prerequisite_skill_ids,<br/>transcript/license/suitability_status,<br/>verification_failures (S27)<br/>learning_sessions: durable session<br/>summary, the 5 orphan fields (U7,<br/>D-331/D-332)<br/>migration_filters: keeps autogenerate<br/>from dropping LangGraph's tables (D-332)<br/>async Alembic migrations"]
