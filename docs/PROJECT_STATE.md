...
| Repo-vs-deployed gap | **0 — repo and staging agree at `523b9f0`** (D-471). The scheduled-job metric filters, heartbeat alarm windows, deploy-role statements, and the D-468 Traceback/collector terraform remain applied via control-plane targeted `terraform apply` (§8) |
...
| `MEMORY-CONSOLIDATION-DEFECTS` (post-migration discovery; evidence: `docs/resume_evidence/04_memory/E4_REPORT.md` §7, D-460; **#1 FIXED 2026-08-30, D-467** — truncation now fails closed at the gateway and the budget was re-derived: facts 6→119 at 119/119 provenance on the same n=10 corpus, 0 silent truncations, 11 honestly-reported failures; evidence `docs/resume_evidence/04_memory/post_remediation/`) | Remaining findings #2–#5: **MEMORY-POLARITY-DEFAULT** (98/120 weak_skill facts leave polarity at schema default → contradiction protocol rarely fires), **MEMORY-STALE-FACT-SERVED** (`top_fact_for_skill` serves the stale higher-confidence positive after a regression, 985/985 — recency is not a ranking term), **MEMORY-CACHE-WRITE-UNBILLED** (~2.8× cost under-report + a never-read prompt cache), and **MEMORY-CEILING-STILL-SATURATED** (D-471 live evidence: the staging fixture cohort at ~26 facts derives 5,888 tokens vs the 4,000 hard cap — 8/8 calls truncated, all surfaced, 0 silent; window bloated by load-test events; bounding the response shape is the open behavior decision) | Fix #2 (prompt/schema tells the model what polarity means) and #3 (recency term in ranking) next; #4 is a cost-accounting fix; the residual-truncation bound is a design decision. Not started | engineering |
