...
resume-evidence program added `CONTENT-GATE-HINT-COHERENCE`, `STAGING-CONN-CEILING`,
`MEMORY-CONSOLIDATION-DEFECTS`, `PII-REDACTION-GAPS`, `HITL-INTERRUPT-HARDENING`, and
`RESUME-EVIDENCE-PROGRAM`, each naming its `docs/resume_evidence/` evidence home inline). The `NO-NEW-TEST-CODE` category is **closed**: all three
