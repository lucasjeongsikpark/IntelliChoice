...
| Last product-code commit | **`05ed0ed`** (2026-08-30, D-467 truncation fail-closed) + the D-468 unhandled-500 logging fix landing after it; before them `f2731a1` (D-457) |
...
| Repo-vs-deployed gap | **11 product commits** (`5fa15d4` → `f2731a1`: the D-450..D-452 e2e specs and comments — test-side — plus the D-457 dependency bumps: nltk 3.10.3 (security, PYSEC-2026-3726), boto3, and dev-tooling; the remediation program CHANGES this: the D-467 gateway fail-closed fix and the D-468 unhandled-500 JSON-ERROR line are product behavior waiting on the next deploy, alongside the nltk security fix). The scheduled-job **metric filters (2026-08-21), heartbeat alarm windows (2026-08-22), and the deploy role's Logs Insights statements (2026-08-24, D-439)** remain applied via control-plane targeted `terraform apply` (§8) |
...
9 open engineering entries (7 in §4.1, 2 in §4.2). Full evidence per entry:
...
resume-evidence program added `OBSERVABILITY-TRACE-GAPS`, `CONTENT-GATE-HINT-COHERENCE`,
`STAGING-CONN-CEILING`, `MEMORY-CONSOLIDATION-DEFECTS`, `PII-REDACTION-GAPS`,
...
|---|---|---|---|
| `OBSERVABILITY-TRACE-GAPS` (post-migration discovery; evidence: `docs/resume_evidence/06_eval_observability/E6_2_REPORT.md`, D-464) | E6.2 measured 100% per-hop trace coverage (12,888/12,888) and, in doing so, found three gaps: **TRACE-ID-COLLISION** (medium) — 0.14% of requests (19/13,550, replicated 1/424 in chat) share an OTel `trace_id` with another request in the same task, so X-Ray merges them and one trace nests two requests; mechanism is a labelled keep-alive-context-leak hypothesis, not verified. **COLLECTOR-STATS-UNSCRAPED: CLOSED 2026-08-30 (D-468)** — the collector's self-telemetry is scraped and four export counters + failure alarms exist in CloudWatch. **CHECKPOINTER-UNINSTRUMENTED** (low) — `AsyncPostgresSaver` uses psycopg directly, so checkpoint I/O has no span | For TRACE-ID-COLLISION: reproduce locally to confirm the context-leak mechanism, then ensure a fresh trace_id per request (the merge silently corrupts trace analysis and any per-request billing/latency join). COLLECTOR-STATS: add one scrape target for `:8888`. CHECKPOINTER: optional span. Not started | engineering |
| `CONTENT-GATE-HINT-COHERENCE` (post-migration discovery; evidence: `docs/resume_evidence/05_content_generation/E5_2_REPORT.md` §finding-1, D-463) | The E5.2 defect benchmark found **`mismatched_hint_ladder` is 0/17 on every detector** (17 of 28 total misses): nothing in the generation gate path relates a hint to its stem, so a hint ladder copied from a different item passes silently. Two adjacent, cheap wins from the same benchmark: wiring the **`arithmetic_identity` check into the dedup stage** (17/17 clone catch vs the 0.05 cosine threshold's 2/6–0/5 on renamed/name+noun clones), and the **6 genuine same-arithmetic duplicate pairs already in the approved bank** it surfaced (a real non-quantity content defect, D-342-reportable) | Decide whether to wire the built-but-unused hint/solution review subsystem (`HINT_SOLUTION_REVIEW.md`) or a deterministic hint-stem check into the gate; and whether to add `arithmetic_identity` to dedup and de-dupe the 6 bank pairs. Low urgency (content-quality, not a live failure). Not started | engineering |
...
| `MEMORY-CONSOLIDATION-DEFECTS` (post-migration discovery; evidence: `docs/resume_evidence/04_memory/E4_REPORT.md` §7, D-460; **#1 FIXED 2026-08-30, D-467** — truncation now fails closed at the gateway and the budget was re-derived: facts 6→119 at 119/119 provenance on the same n=10 corpus, 0 silent truncations, 11 honestly-reported failures; evidence `docs/resume_evidence/04_memory/post_remediation/`) | Remaining findings #2–#5: **MEMORY-POLARITY-DEFAULT** (98/120 weak_skill facts leave polarity at schema default → contradiction protocol rarely fires), **MEMORY-STALE-FACT-SERVED** (`top_fact_for_skill` serves the stale higher-confidence positive after a regression, 985/985 — recency is not a ranking term), **MEMORY-CACHE-WRITE-UNBILLED** (~2.8× cost under-report + a never-read prompt cache), and the 11/30 calls that still truncate at the new 2560 budget (now reported, not silent — bounding the response shape is an open behavior decision) | Fix #2 (prompt/schema tells the model what polarity means) and #3 (recency term in ranking) next; #4 is a cost-accounting fix; the residual-truncation bound is a design decision. Not started | engineering |
...
| Register key | What it is | Remaining action | Owner |
|---|---|---|---|
| `RESUME-EVIDENCE-PROGRAM` (post-migration discovery; evidence: `docs/resume_evidence/MEASUREMENT_PLAN.md`, D-458) | User-commissioned 2026-08-28: a six-theme measurement program producing artifact-backed resume evidence. Tier 2 (~$5–9 real Bedrock) and Tier 3 (the isolated 200-candidate generation benchmark, ~$5–10) are **user-authorized**; Tier 3 is isolated-DB quality measurement, outside D-342's subject. Executed via Orca, one opus/high executor per experiment; findings reported-not-fixed inside measurement tasks | COMPLETE — all nine experiments accepted (E5.1/E6.1/E3/E4/E1/E2/E5.2/E6.2/E5.3, D-458..D-465) and both synthesis documents written (`RESUME_METRICS_REPORT.md` with the summary table + A-D ranking, `RECOMMENDED_RESUME_BULLETS.md`), D-466. The program leaves five open follow-up remediation rows (§4.1: OBSERVABILITY-TRACE-GAPS, CONTENT-GATE-HINT-COHERENCE, STAGING-CONN-CEILING, MEMORY-CONSOLIDATION-DEFECTS, PII-REDACTION-GAPS) — none blocking, all reported-not-fixed | engineering |
...
| 2 | `PII-REDACTION-GAPS` | Added 2026-08-29 (D-458/E6.1): concrete, cheap fixes with a measured baseline already recorded — F-1 case-insensitive URL matching, F-2 span-redactor credential vocabulary rejoined with the log denylist's. Fix AFTER the program's measurement tasks so baselines stay stable |
