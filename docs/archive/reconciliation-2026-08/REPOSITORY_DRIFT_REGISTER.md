...
> **Reason:** Phase 3A repository drift register — the 102 documented-intent-vs-observed-code rows read at HEAD `344f016`. **Superseded by:** the 166-entry register at `docs/reference/reconciliation-2026-08/FINAL_OPEN_WORK_REGISTER.md`.
...
- **Annotation 2026-08-20 (step 7d, W-14): the count in the observed state is off by one — the grep returns FOUR lines, not three.** Re-run 2026-08-20, `grep -rn "checkpoint_repair" terraform/` → `terraform/modules/ecs-service/main.tf:209`, `main.tf:255`, `terraform/modules/observability/dashboard.tf:425` (the AUD-X-07 rationale comment, which the entry cites as `:425-427` but does not count) and `dashboard.tf:436`. The undercount comes from counting cited *artifacts* as grep *lines*. **The finding itself is unchanged and stands:** `alarms.tf` contains no reference, so nothing can page on the counter whose movement voids the §7-R9 acceptance.
...
- **Annotation 2026-08-20 (step 7d, W-14): standing note — "zero blank/stuck states enforced at teardown" overstates the harness in TWO independent ways, and the second is the one that bites.** (1) Blank/stuck is **not** a teardown assertion at all: it is the `pageerror` listener plus the in-test helpers `expectNotBlank`/`expectNotStuck`, so it holds only where a spec calls them. (2) **`assertClean()` runs only when `testInfo.status === "passed"`** — so on a *failing* test the criterion-3 evidence is captured and persisted but **never enforced**, which is precisely the run where a blank or stuck state is most likely. Any doc wording that keeps "enforced at teardown" must add "on passing tests only". Related discipline (`WHOLE-SUITE-NOT-RUN`): "the suite is green" is not "everything run passed".
...
- **Annotation 2026-08-20 (step 7d, W-14): the 41-vs-31 correction is re-verified, and it may only ever be restated WITH its denominator and its mechanism caveat.** Re-run 2026-08-20: `grep -r 'model_config = ConfigDict(extra="forbid")' apps packages --include="*.py" | wc -l` → **41**, distributed exactly as the observed state describes (`bedrock.py` 35, `adjudications.py` 2, `authored_bank.py` 2, `smoke_cli.py` 2). **Required denominator: 41 of 184 classes — 22%.** A bare "41 models are `extra="forbid"`" reads as coverage and is how this row drifted the first time; 78% of classes are *not* under the fence. **Required caveat, restated so it cannot be dropped: `pyright` does not fail if an `extra="forbid"` is deleted** — clause (b) therefore holds only for the Bedrock-payload subset pinned by `test_bedrock_payload_pii_floor.py:26-32`, and not for the other six models. Whoever restates this figure carries both halves or the row drifts again.
