...
2. Difficulty matching estimated level - each target skill's starting question is served
   at its own difficulty tier, and `recommended_difficulty` (from bootstrap mastery) seeds
   the session's `starting_difficulty`. With the current 1:1 skill<->difficulty curriculum,
   a skill *is* its difficulty tier.
3. Difficulty within +/-1 - the retry ladder's prerequisite step drops a skill line one
   tier via `curriculum.prerequisite_for` (see `flow._advance_study`); `_select_template`
   falls back across a skill's whole approved pool when a specific variant is unavailable.
4. Template not yet used this session - tracked via `used_template_ids`.
...
async def _select_template(
    ...
    """Pick an approved template for `skill_id`, preferring one not yet used this session
    (§5.11.2 rule 4), falling back to the full approved pool if all have been used.
    """
    ...
    unused = [t for t in candidates if t.question_template_id not in used_template_ids]
...
async def create_study_item(
    *,
    ...
    """
    template = await _select_template(question_repo, skill_id, used_template_ids, rng)
