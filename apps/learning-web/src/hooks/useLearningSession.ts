...
export function useLearningSession(
  ...
  // back. `EventSource` retries *network* errors on its own backoff, including a connection
  // that never established (measured in the e2e harness 2026-08-26: 1 -> 4 attempts over 12s
  // of idle behind an aborted route - this clause read "only after a *successful* connection
  // drops" until then). What it does not retry is a non-2xx response (an expired token, a
  // 403), which is terminal, and before this the app had no path to a fresh connection short
  // of a full reload.
