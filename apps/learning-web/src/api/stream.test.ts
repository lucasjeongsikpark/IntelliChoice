 ...
 *
 * The learning-web half of D-405, and it exists because of D-347 rather than because a walk found
 * something here. `EDGE-CHAT-02` was filed against chat, but both apps run the same `EventSource`
 * shape against the same keepalive, so a timer in one and not the other is the "fixed in one
 * direction" pattern the 08-16 audit called the finding behind its findings - and learning-web is
 * the app where a stale stream costs a student their exam snapshot rather than a chat answer.
 *
 * learning-web already had the reconnect *control* (D-216); what it never had, and neither app
 * did, is a way to notice it needs using.
 *
...
describe("openSessionStream liveness", () => {
  ...
  function open() {
    const states: string[] = [];
    const close = openSessionStream("session-1", "token-1", () => {}, (s) => states.push(s));
  ...
  it("a snapshot also counts as proof of life", () => {
    ...
    vi.advanceTimersByTime(STALE_AFTER_MS - 1000);
    source.onmessage?.({ data: JSON.stringify({ phase: "study" }) });
    vi.advanceTimersByTime(STALE_AFTER_MS - 1000);
